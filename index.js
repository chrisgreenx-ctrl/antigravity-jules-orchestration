...
app.get('/mcp/tools', (req, res) => {
  res.json({
    tools: [
      ...
      // v2.4.0: Session Management
      {
        name: 'jules_cancel_session',
        description: 'Cancel/abort an active Jules session',
        parameters: { sessionId: { type: 'string', required: true, description: 'Session ID to cancel' } }
      },
      {
        name: 'jules_retry_session',
        description: 'Retry a failed session with same or modified config',
        parameters: {
          sessionId: { type: 'string', required: true, description: 'Failed session ID to retry' },
          modifiedPrompt: { type: 'string', required: false, description: 'Optional modified prompt' }
        }
      },
      {
        name: 'jules_get_diff',
        description: 'Get code diff/changes from a completed session',
        parameters: { sessionId: { type: 'string', required: true, description: 'Session ID' } }
      },
      {
        name: 'jules_list_batches',
        description: 'List all batch operations with their status',
        parameters: {}
      },
      {
        name: 'jules_delete_session',
        description: 'Delete a session (cleanup)',
        parameters: { sessionId: { type: 'string', required: true, description: 'Session ID to delete' } }
      },
      {
        name: 'jules_clear_cache',
        description: 'Clear the API response cache',
        parameters: {}
      },
      {
        name: 'jules_cache_stats',
        description: 'Get cache statistics',
        parameters: {}
      },
      {
        name: 'jules_cancel_all_active',
        description: 'Cancel all active sessions (emergency stop)',
        parameters: { confirm: { type: 'boolean', required: true, description: 'Must be true to confirm' } }
      },
      // v2.5.0: Session Templates
      {
        name: 'jules_create_template',
        description: 'Save a session configuration as a reusable template',
        parameters: {
          ...
          config: { type: 'object', required: true, description: 'Session config (prompt, source, title, etc.)' }
        }
      },
      {
        name: 'jules_list_templates',
        description: 'List all saved session templates',
        parameters: {}
      },
      {
        name: 'jules_create_from_template',
        description: 'Create a session from a saved template',
        parameters: {
          templateName: { type: 'string', required: true, description: 'Template name to use' },
          overrides: { type: 'object', required: false, description: 'Override template values' }
        }
      },
      {
        name: 'jules_delete_template',
        description: 'Delete a saved template',
        parameters: { name: { type: 'string', required: true, description: 'Template name' } }
      },
      // v2.5.0: Session Cloning
      {
        name: 'jules_clone_session',
        description: 'Clone a session configuration to create a new similar session',
        parameters: {
          sessionId: { type: 'string', required: true, description: 'Session ID to clone' },
          modifiedPrompt: { type: 'string', required: false, description: 'Modified prompt for the clone' },
          newTitle: { type: 'string', required: false, description: 'Title for cloned session' }
        }
      },
      // v2.5.0: Session Search
      {
        name: 'jules_search_sessions',
        description: 'Search sessions with filters (title, state, date range)',
        parameters: {
          query: { type: 'string', required: false, description: 'Search query for title' },
          state: { type: 'string', required: false, description: 'Filter by state (COMPLETED, FAILED, IN_PROGRESS, etc.)' },
          limit: { type: 'number', required: false, description: 'Max results (default: 20)' }
        }
      },
      // v2.5.0: PR Integration
      {
        name: 'jules_get_pr_status',
        description: 'Get detailed PR status from a completed session',
        parameters: { sessionId: { type: 'string', required: true, description: 'Session ID' } }
      },
      {
        name: 'jules_merge_pr',
        description: 'Merge a PR created by Jules',
        parameters: {
          owner: { type: 'string', required: true, description: 'Repository owner' },
          repo: { type: 'string', required: true, description: 'Repository name' },
          prNumber: { type: 'number', required: true, description: 'PR number to merge' },
          mergeMethod: { type: 'string', required: false, description: 'merge, squash, or rebase (default: squash)' }
        }
      },
      {
        name: 'jules_add_pr_comment',
      ...
      {
        name: 'jules_queue_session',
        description: 'Queue a session for later execution with priority',
        parameters: {
          config: { type: 'object', required: true, description: 'Session config' },
          priority: { type: 'number', required: false, description: 'Priority (1-10, lower=higher priority)' }
        }
      },
      {
        name: 'jules_get_queue',
        description: 'Get current session queue',
        parameters: {}
      },
      {
        name: 'jules_process_queue',
        description: 'Process next item in the queue',
        parameters: {}
      },
      {
        name: 'jules_clear_queue',
        description: 'Clear all pending items from the queue',
        parameters: {}
      },
      // v2.5.0: Batch Retry
      {
        name: 'jules_batch_retry_failed',
        description: 'Retry all failed sessions in a batch',
        parameters: {
          batchId: { type: 'string', required: true, description: 'Batch ID' }
        }
      },
      // v2.5.0: Analytics
      {
        name: 'jules_get_analytics',
        description: 'Get session analytics (success rate, timing, trends)',
        parameters: {
          days: { type: 'number', required: false, description: 'Days to analyze (default: 7)' }
        }
      }
    ]
  });
...
function julesRequest(method, path, body = null, opts = {}) {
  const { useCache = true, cacheTTL = 10000 } = opts;
  const cacheKey = `${method}:${path}`;
  const correlationId = `jules-${Date.now()}-${Math.random().toString(36).substr(2, 6)}`;
  ...
  if (method === 'GET' && useCache) {
    ...
    if (cached) {
      structuredLog('debug', 'Cache hit', { correlationId, path });
  ...
  if (method !== 'GET') {
    ...
    if (sessionMatch) apiCache.invalidate(sessionMatch[1]);
  ...
  return new Promise((resolve, reject) => {
    ...
    structuredLog('debug', 'API request', { correlationId, method, path });
      ...
      response.on('end', () => {
        if (response.statusCode >= 200 && response.statusCode < 300) {
          ...
          try {
            ...
            if (method === 'GET' && useCache) {
              apiCache.set(cacheKey, parsed, cacheTTL);
              structuredLog('debug', 'Cached response', { correlationId, path });
            }
            resolve(parsed);
          } catch {
            resolve(data);
...
async function cancelSession(sessionId) {
  ...
  return await retryWithBackoff(
    () => julesRequest('POST', `/sessions/${sessionId}:cancel`, {}),
    { maxRetries: 2, correlationId: `cancel-${sessionId}` }
  );
}

async function retrySession(sessionId, modifiedPrompt = null) {
  structuredLog('info', 'Retrying session', { sessionId, hasModifiedPrompt: !!modifiedPrompt });
  ...
  return await createJulesSession(newConfig);
}

async function getSessionDiff(sessionId) {
  structuredLog('info', 'Getting session diff', { sessionId });
  ...
  return {
    sessionId, state: session.state, title: session.title, prUrl,
    prCreated: !!prUrl, changes: session.changes || null,
    filesModified: session.filesModified || [],
    summary: prUrl ? `Changes available at: ${prUrl}` : 'No PR created yet'
  };
}

async function deleteSession(sessionId) {
  structuredLog('info', 'Deleting session', { sessionId });
  apiCache.invalidate(sessionId);
  return await retryWithBackoff(
    () => julesRequest('DELETE', `/sessions/${sessionId}`),
    { maxRetries: 2, correlationId: `delete-${sessionId}` }
  );
}

async function cancelAllActiveSessions(confirm) {
  if (!confirm) throw new Error('Must pass confirm: true to cancel all sessions');
  structuredLog('warn', 'Emergency: Cancelling all active sessions');
  const sessions = await sessionMonitor.getActiveSessions();
  const results = await Promise.all(
    sessions.map(async (session) => {
      const id = session.name?.split('/').pop() || session.id;
      try {
        await julesRequest('POST', `/sessions/${id}:cancel`, {});
        return { id, cancelled: true };
      } catch (error) {
        return { id, cancelled: false, error: error.message };
      }
    })
  );
  apiCache.clear();
...
function createTemplate(name, description, config) {
  if (!name || !config) throw new Error('Template name and config are required');
  ...
  structuredLog('info', 'Template created', { templateName: name });
  return { success: true, template };
}

function listTemplates() {
  const templates = Array.from(sessionTemplates.values()).map(t => ({
    name: t.name,
...
async function createFromTemplate(templateName, overrides = {}) {
  ...
  template.usageCount++;
  structuredLog('info', 'Creating session from template', { templateName, overrides: Object.keys(overrides) });
  return await createJulesSession(config);
}

function deleteTemplate(name) {
  if (!sessionTemplates.has(name)) throw new Error(`Template "${name}" not found`);
  sessionTemplates.delete(name);
  structuredLog('info', 'Template deleted', { templateName: name });
...
async function cloneSession(sessionId, modifiedPrompt = null, newTitle = null) {
  structuredLog('info', 'Cloning session', { sessionId });
  const original = await julesRequest('GET', `/sessions/${sessionId}`);
  if (!original) throw new Error(`Session ${sessionId} not found`);
  const cloneConfig = {
  ...
  const newSession = await createJulesSession(cloneConfig);
  return {
    originalSessionId: sessionId,
    newSession,
    clonedFrom: { title: original.title, state: original.state }
  };
}
...
async function searchSessions(query = null, state = null, limit = 20) {
  structuredLog('info', 'Searching sessions', { query, state, limit });
  const allSessions = await julesRequest('GET', '/sessions');
  let sessions = allSessions.sessions || [];
  if (state) {
    sessions = sessions.filter(s => s.state === state.toUpperCase());
  }
  if (query) {
...
async function getPrStatus(sessionId) {
  structuredLog('info', 'Getting PR status', { sessionId });
  const session = await julesRequest('GET', `/sessions/${sessionId}`);
  const activities = await julesRequest('GET', `/sessions/${sessionId}/activities`);
  const prActivity = activities.activities?.find(a => a.prCreated);
  if (!prActivity) {
    return { sessionId, prCreated: false, message: 'No PR created for this session' };
  }
  const prUrl = prActivity.prCreated.url;
  ...
  return {
    ...
    owner: prMatch?.[1],
    repo: prMatch?.[2],
    prNumber: prMatch?.[3] ? parseInt(prMatch[3]) : null,
...
async function mergePr(owner, repo, prNumber, mergeMethod = 'squash') {
  ...
  structuredLog('info', 'Merging PR', { owner, repo, prNumber, mergeMethod });
  return new Promise((resolve, reject) => {
    const options = {
    ...
    const req = https.request(options, (res) => {
      ...
      res.on('end', () => {
        if (res.statusCode >= 200 && res.statusCode < 300) {
          resolve({ success: true, merged: true, prNumber, ...JSON.parse(data || '{}') });
        } else {
...
async function addPrComment(owner, repo, prNumber, comment) {
  if (!GITHUB_TOKEN) throw new Error('GITHUB_TOKEN not configured');
  structuredLog('info', 'Adding PR comment', { owner, repo, prNumber });
  return new Promise((resolve, reject) => {
    const options = {
      hostname: 'api.github.com',
...
async function processQueue() {
  ...
  structuredLog('info', 'Processing queue item', { queueId: next.id, title: next.config.title });
  try {
...
async function batchRetryFailed(batchId) {
  structuredLog('info', 'Retrying failed sessions in batch', { batchId });
  ...
  if (failedTasks.length === 0) {
    return { message: 'No failed sessions to retry', batchId };
  }
  const retryResults = await Promise.all(
    failedTasks.map(async (task) => {
      try {
        const newSession = await retrySession(task.sessionId || task.id);
        return { originalId: task.sessionId || task.id, newSessionId: newSession.name || newSession.id, success: true };
      } catch (error) {
        return { originalId: task.sessionId || task.id, success: false, error: error.message };
      }
    })
  );
  return {
...
async function getAnalytics(days = 7) {
  structuredLog('info', 'Getting analytics', { days });
  const allSessions = await julesRequest('GET', '/sessions');
  const sessions = allSessions.sessions || [];
  const cutoffDate = new Date();
  cutoffDate.setDate(cutoffDate.getDate() - days);
  const recentSessions = sessions.filter(s => {
    const created = new Date(s.createTime || s.createdAt);
